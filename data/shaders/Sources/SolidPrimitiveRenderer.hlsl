...
struct PS_INPUT
{
	...
	uint fVertex					: TEXCOORD2;
};

void VS_Render(in VS_INPUT_INSTANCED input, uint ahaha : SV_VertexID, out PS_INPUT output)
{
	float4 vInstPosition = mul(float4(input.vPosition, 1.0f), input.mInstance);

	output.vProjPos = mul(vInstPosition, g_mViewProj);
	output.fVertex = ahaha;
...
float4 PS_Render(PS_INPUT input) : SV_TARGET
{
	...
	//vColor.xyz = input.fVertex * 0.01f;

	//vColor = float4(input.vTexCoord, 0.0f, 1.0f);

	vColor.xyz *= max(0.0f, dot(-float3(-0.5f, -0.5f, 0.5f), input.vWorldNormal));
	vColor.xyz += 0.0f;
